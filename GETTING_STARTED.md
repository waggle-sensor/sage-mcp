...
The SAGE MCP now includes a powerful plugin generation feature that allows you to create new plugins using natural language descriptions. Here's how to use it:

### Basic Plugin Creation

```python
from sage_mcp import create_plugin

# Create a basic plugin with automatic requirement detection
result = create_plugin(
    description="A plugin that uses computer vision to detect and count flowering plants in camera images",
...
You can specify custom requirements using a JSON string:

```python
result = create_plugin(
    description="A plugin that uses deep learning to analyze bird sounds and identify species",
    name="Bird Species Detector",
    requirements='{"gpu": true, "audio": true, "python_packages": ["torch", "librosa", "tensorflow"]}'
)
```
...
pluginctl build .
pluginctl run .  # Test locally
pluginctl deploy .  # Deploy to SAGE nodes
```
